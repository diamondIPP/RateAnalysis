...
class PadAnalysis(DUTAnalysis):
    ...
    # ----------------------------------------
    # region CUTS
    def draw_bucket_histo(self, show=True):
        x = self.get_root_vec(var=self.get_signal_var(), cut=self.Cut.get_pre_bucket())
        xmax = quantile(x, .95)
        self.Draw.distribution(x, self.Bins.get_pad_ph(max(1, xmax / 40)), 'Bucket Events', y_tit='Pulse Height [mV]', show=show, x_range=[-50, xmax])
    ...
    def print_bucket_events(self, prnt=True, redo=False):
        def f():
            ...
            return self.get_n_entries(self.Cut.get_pre_bucket()), self.get_n_entries(self.Cut.get_bucket())
        n_pre, n_new = do_pickle(self.make_simple_pickle_path('BucketEvents', sub_dir='Cuts'), f, redo=redo)
    ...
    def draw_bucket_map(self, pre=False, res=None, show=True):
        h = self.draw_hitmap(cut=self.Cut.get_pre_bucket() if pre else self.Cut.get_bucket(), show=show, res=res)
    ...
    def draw_bucket_pedestal(self, show=True, corr=True, additional_cut=''):
        xbins = Bins.make(*array(self.Run.IntegralRegions[self.DUT.Number - 1]['signal_e']) * self.DigitiserBinWidth, bin_width=.5)
        cut_string = self.Cut.generate_custom(exclude='bucket') + TCut(additional_cut)
        self.draw_ph_peaktime(region='e', cut=cut_string, fine_corr=corr, prof=False, show=show, xbins=xbins, logz=True)

    def draw_bucket_distributions(self):
        cuts = [self.Cut.generate_custom(exclude='bucket', name='nobucket'), self.Cut.generate_custom(exclude='bucket', name='prebucket') + self.Cut.generate_pre_bucket()(), None]
        histos = [self.draw_signal_distribution(cut=cut, show=False) for cut in cuts]
    ...
    def compare_single_cuts(self, scale=True, redo=False):
        histos = [self.draw_signal_distribution(cut(), show=False, redo=redo, save=False) for cut in self.Cut.get_strings()]
        self.Draw.stack(histos, 'Single Cuts', self.Cut.get_names(), scale=scale)

    def compare_consecutive_cuts(self, scale=False, short=False, x_range=None, redo=False):
        cuts = self.Cut.get_consecutive(short)
        histos = [self.draw_signal_distribution(cut=cut, show=False, redo=redo, x_range=x_range, save=False) for cut in cuts.values()]
    ...
    def draw_cut_means(self, short=False, redo=False, show=True):
        y = [self.get_pulse_height(cut=cut, redo=redo) for cut in self.Cut.get_consecutive(short).values()]
        ...
        for i, name in enumerate(self.Cut.get_consecutive(short).keys()):
            g.GetXaxis().SetBinLabel(g.GetXaxis().FindBin(i), name)
        update_canvas()

    def draw_signal_vs_signale(self, show=True):
        x, y = self.get_root_vec(var=[self.get_signal_var(), self.get_signal_var(region='e')], cut=self.Cut.generate_custom(exclude='bucket'))
        names = self.SignalRegionName.title().replace('_', ' '), 'Signal E'
        self.Draw.histo_2d(x, y, self.Bins.get_pad_ph(mean_ph=mean(x)) * 2, '{} vs. {}'.format(*names), x_tit='{} [mV]'.format(names[0]), y_tit='{} [mV]'.format(names[1]), pal=53, stats=0, show=show,
                           z_off=1.4, logz=True)

    def draw_cut_vars(self, normalise=False, consecutive=False):
        values = [self.get_ph_values(cut=(self.Cut.generate_threshold() + cut).Value) for cut in (self.Cut.get_consecutive().values() if consecutive else self.Cut.get_strings(with_raw=True))]
        ...
        names = self.Cut.get_names(with_raw=True)
