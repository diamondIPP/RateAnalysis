...
from run import Run
from utils import has_bit, critical, warning
...
class PadRun(Run):
    ...
    def __init__(self, run_number=None, test_campaign=None, tree=None, t_vec=None, verbose=False):
        ...
        self.Digitiser = self.Config.get('BASIC', 'digitizer')
...
if __name__ == '__main__':
    ...
    parser.add_argument('run', nargs='?', default=23, type=int)
    parser.add_argument('-tc', '--testcampaign', nargs='?', default='201908')
    parser.add_argument('-t', '--tree', action='store_true')
    args = parser.parse_args()
    z = PadRun(args.run, tree=None if not args.tree else False, test_campaign=args.testcampaign)
