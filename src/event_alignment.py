...
class EventAligment(object):
    ...
    def check_alignment(self, *args, **kwargs):
        ...
        warning(f'{calc_eff(values=invert(v))[0]:.1f}% of the events are misaligned :-(', prnt=self.Run.Verbose) if not all(v) else self.Run.info(f'Run {self.Run.Number} is perfectly aligned :-)')
        return all(v)
