...
class SaveDraw(Draw):
    ...
    def save_plots(self, savename, sub_dir=None, canvas=None, prnt=True, show=True):
        ...
        try:
            self.save_canvas(canvas, sub_dir=sub_dir, file_name=savename, prnt=prnt, show=show)
            return Draw.add(canvas)
        except Exception as inst:
            warning('Error saving plots ...:\n  {}'.format(inst))

    def save_canvas(self, canvas, file_name, res_dir=None, sub_dir=None, ftype=None, prnt=True, show=True):
